...
public class DBHelper {
    ...
    private static String creatTableReviewSql = "create table if not exists review" +
            "(" +
            "    movie_title text, " +
            "    total integer, " +
            "    writer text, " +
    ...
    public static ArrayList<?> selectTable(String tableName) {
        ...
        if (database != null) {
            switch (tableName) {
                ...
                case "review":
                    sql = "select movie_title, total, writer, review_id, writer_image, time, time_stamp, rating, contents, recommend from "
                            + tableName;
    ...
    public static void insertReviewData(ArrayList<ReviewItem> items) {
        try {
            for (int i = 0; i < items.size(); i++) {
                String sql = "insert into review(movie_title, total, writer, review_id, writer_image, time, time_stamp, rating, contents, recommend) " +
                        "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
